...
export const ShakeGameList = () => {
  const { data: shakeGames } = useGames("shake");
  const gamesArray = Array.isArray(shakeGames)
    ? shakeGames
    : [shakeGames].filter(Boolean);
  ...
  return (
    <div>
      <div className="flex justify-between items-center my-6">
        <h1 className="text-2xl font-bold">Shake Games</h1>
      </div>
      {shakeGames === undefined ? (
        <Loader />
      ) : gamesArray.length === 0 ? (
        <Card className="w-full">
          <CardContent className="flex flex-col items-center justify-center py-10">
            <p className="text-lg mb-4">
              No shake games have been created yet.
            </p>
          </CardContent>
        </Card>
      ) : (
